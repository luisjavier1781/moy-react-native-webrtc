...
export default class MediaStreamTrack extends EventTarget<MediaStreamTrackEventMap> {
    ...
    /**
     * Sets the zoom level. Returns null on success, or a diagnostic string
     * describing why zoom failed (e.g. reflection field not found).
     */
    async _setZoom(zoomLevel: number): Promise<string | null> {
        if (this.remote) {
            throw new Error('Not implemented for remote tracks');
        }
        ...
        return WebRTCModule.mediaStreamTrackSetZoom(this.id, zoomLevel);
    }

    async _getZoomRange(): Promise<{ min: number; max: number }> {
        if (this.remote) {
            throw new Error('Not implemented for remote tracks');
        }
        ...
        return WebRTCModule.mediaStreamTrackGetZoomRange(this.id);
    }
