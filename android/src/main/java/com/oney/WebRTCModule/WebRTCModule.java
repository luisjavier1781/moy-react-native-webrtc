...
public class WebRTCModule extends ReactContextBaseJavaModule {
    ...
    @ReactMethod
    public void mediaStreamTrackSetZoom(String id, float zoomRatio, Promise promise) {
        ThreadUtils.runOnExecutor(() -> {
            // Returns null on success or a diagnostic error string on failure
            String diagnostic = getUserMediaImpl.setZoom(id, zoomRatio);
            promise.resolve(diagnostic);
        });
    }

    @ReactMethod
    public void mediaStreamTrackGetZoomRange(String id, Promise promise) {
        ThreadUtils.runOnExecutor(() -> {
            float[] range = getUserMediaImpl.getZoomRange(id);
            com.facebook.react.bridge.WritableMap result = com.facebook.react.bridge.Arguments.createMap();
            result.putDouble("min", range[0]);
            result.putDouble("max", range[1]);
