...
RCT_EXPORT_METHOD(mediaStreamTrackSetZoom : (nonnull NSString *)trackID : (double)zoomRatio : (RCTPromiseResolveBlock)resolve : (RCTPromiseRejectBlock)reject) {
#if TARGET_OS_TV
    reject(@"unsupported_platform", @"tvOS is not supported", nil);
    return;
#else
...
RCT_EXPORT_METHOD(mediaStreamTrackGetZoomRange : (nonnull NSString *)trackID : (RCTPromiseResolveBlock)resolve : (RCTPromiseRejectBlock)reject) {
#if TARGET_OS_TV
    reject(@"unsupported_platform", @"tvOS is not supported", nil);
    return;
#else
    ...
    if (track && [track.kind isEqualToString:@"video"]) {
        ...
        if ([videoTrack.captureController isKindOfClass:[VideoCaptureController class]]) {
            VideoCaptureController *vcc = (VideoCaptureController *)videoTrack.captureController;
            resolve([vcc getZoomRange]);
        } else {
            resolve(@{@"min": @(1.0), @"max": @(1.0)});
        }
    } else {
        reject(@"E_INVALID", @"Track not found or not a video track", nil);
    }
