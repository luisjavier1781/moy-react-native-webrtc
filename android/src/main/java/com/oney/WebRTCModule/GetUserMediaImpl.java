...
class GetUserMediaImpl {
    ...
    // -------------------------------------------------------------------------
    // Zoom API — delegates to CameraCaptureController / ZoomController
    // -------------------------------------------------------------------------

    /**
     * Sets the zoom level for the given video track.
     * Only works for Camera2-based tracks; no-op otherwise.
     *
     * @param trackId   the MediaStreamTrack id
     * @param zoomRatio desired zoom (1.0 = no zoom)
     */
    String setZoom(String trackId, float zoomRatio) {
        TrackPrivate track = tracks.get(trackId);
        if (track != null && track.videoCaptureController instanceof CameraCaptureController) {
            return ((CameraCaptureController) track.videoCaptureController).setZoom(zoomRatio);
        } else {
            String msg = "ZOOM_NO_TRACK: track not found or not Camera2: " + trackId;
            Log.w(TAG, msg);
            return msg;
        }
    }
    ...
    /**
     * Returns [minZoom, maxZoom] for the given video track.
     * Returns [1.0, 1.0] when track is not found or zoom not supported.
     *
     * @param trackId the MediaStreamTrack id
     */
    float[] getZoomRange(String trackId) {
        TrackPrivate track = tracks.get(trackId);
        if (track != null && track.videoCaptureController instanceof CameraCaptureController) {
